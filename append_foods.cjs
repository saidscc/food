...
// Also, let's update some of the existing meals to recommend the new sides & salads.
code = code.replace(
  'id: "plov",',
...
// We need to inject the newFoodsString into the array before the last `];`
// Find the last index of `];` in code
const lastIndex = code.lastIndexOf('];');
if (lastIndex !== -1) {
  const finalCode = code.slice(0, lastIndex) + ',\n' + newFoodsString + '\n' + code.slice(lastIndex);
  fs.writeFileSync('src/lib/foods.ts', finalCode);
  console.log('Appended new foods successfully.');
} else {
